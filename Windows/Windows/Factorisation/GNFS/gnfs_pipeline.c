...
static FactorResultBigNum cpss_factor_gnfs_pipeline(const BigNum* n, const GNFSConfig* cfg) {
    ...
    for (int ri = 0; ri < rfb_count && rfb[ri] <= (uint64_t)cfg->afb_bound; ++ri) {
        uint64_t p = rfb[ri];
        /* Find roots of f(x) mod p by brute force */
    ...
    /* Total columns: 2 sign columns + rational FB + algebraic FB + character + optional Schirokauer
     * Column 0 = rational sign: sign(a - b*m)
     * Column 1 = algebraic sign: sign(a - b*alpha_real)  [infinite-place character] */
    int sign_col = 2; /* columns 0 and 1 are sign columns */
    ...
    if (cfg->diag) {
        printf("  [COL-LAYOUT] rat_sign=0 alg_sign=1  RFB=[%d..%d](%d)  AFB=[%d..%d](%d)  CHAR=[%d..%d](%d)  SCHK2=[%d..%d](%d)  SCHK=[%d..%d](%d)  total=%d\n",
        ...
        printf("]\n");
        printf("  [GNFS] total columns: %d (2 sign + %d rational + %d algebraic + %d character + %d schk2 + %d schk_ell)\n",
            total_cols, rfb_count, afb_count, char_count, schk2_cols, schk_ell_cols);
    ...
    if (cfg->variant_sweep && rel_count > 0) {
        ...
        printf("  [VARIANT-SWEEP] relation pool: %d relations\n", rel_count);
        ...
        printf("  [VARIANT-SWEEP] test primes: %d\n", vt_np);
        ...
        for (int vi = 0; vi < n_variants; ++vi) {
            ...
            for (int ri = 0; ri < rel_count; ++ri) {
                ...
                if(v_schk3_cols>0){
                    ...
                    uint64_t f2e3=bn_mod_u64(&poly.coeffs[2],el3),f3e3=bn_mod_u64(&poly.coeffs[3],el3);
                    uint64_t a2_3=mulmod_u64(aell3,aell3,el3),b2_3=mulmod_u64(bell3,bell3,el3);
                    uint64_t nm3=(mulmod_u64(f3e3,mulmod_u64(a2_3,aell3,el3),el3)+mulmod_u64(f2e3,mulmod_u64(a2_3,bell3,el3),el3)
                        +mulmod_u64(f1e3,mulmod_u64(aell3,b2_3,el3),el3)+mulmod_u64(f0e3,mulmod_u64(b2_3,bell3,el3),el3))%el3;
                    if(nm3!=0){uint64_t fm3[3];for(int k=0;k<3;++k)fm3[k]=bn_mod_u64(&poly.coeffs[k],el3_2);
                        uint64_t th3[3];th3[0]=(uint64_t)(((a%(int64_t)el3_2)+(int64_t)el3_2)%(int64_t)el3_2);th3[1]=(el3_2-(b%el3_2))%el3_2;th3[2]=0;
                        uint64_t s3e=(el3*el3-1u)*(el3*el3+el3+1u),s3r[3]={1,0,0},s3bb[3]={th3[0],th3[1],th3[2]};
                        while(s3e>0u){if(s3e&1u){uint64_t _t[5]={0,0,0,0,0};for(int _i=0;_i<3;++_i)for(int _j=0;_j<3;++_j)_t[_i+_j]=(_t[_i+_j]+mulmod_u64(s3r[_i],s3bb[_j],el3_2))%el3_2;
                        ...
                        {uint64_t _t[5]={0,0,0,0,0};for(int _i=0;_i<3;++_i)for(int _j=0;_j<3;++_j)_t[_i+_j]=(_t[_i+_j]+mulmod_u64(s3bb[_i],s3bb[_j],el3_2))%el3_2;
                            if(_t[4]){_t[3]=(_t[3]+el3_2-mulmod_u64(_t[4],fm3[2],el3_2))%el3_2;_t[2]=(_t[2]+el3_2-mulmod_u64(_t[4],fm3[1],el3_2))%el3_2;_t[1]=(_t[1]+el3_2-mulmod_u64(_t[4],fm3[0],el3_2))%el3_2;}
                            if(_t[3]){_t[2]=(_t[2]+el3_2-mulmod_u64(_t[3],fm3[2],el3_2))%el3_2;_t[1]=(_t[1]+el3_2-mulmod_u64(_t[3],fm3[1],el3_2))%el3_2;_t[0]=(_t[0]+el3_2-mulmod_u64(_t[3],fm3[0],el3_2))%el3_2;}
                            s3bb[0]=_t[0];s3bb[1]=_t[1];s3bb[2]=_t[2];}s3e>>=1u;}
            ...
            int v_ns=0;
            for(int r=0;r<rel_count;++r){bool zr=true;for(int w=0;w<v_rw;++w)if(vm[r*v_rw+w]){zr=false;break;}if(zr)++v_ns;}
            /* Local-square on first 50 null-space deps */
            int v_pass=0, v_total_qr=0, v_total_tests=0, di_tested=0;
            for (int r = 0; r < rel_count && di_tested < 50; ++r) {
            ...
            free(v_row_used); free(v_hist); free(v_pivot); free(vm);
        }
            ...
            { uint64_t rng = 0xC0FFEE1234567891ULL ^ (uint64_t)bn_bitlen(n);
              ...
              for (; tries < 2000 && aux_prime_count < 5; ++tries) {
                   ...
                   * This uses O(log p) polynomial multiplications, each O(d²). */
                  {
                      ...
                      #undef POLY3_MUL
                      /* Now res = x^p mod f(x) mod p. f is irreducible iff x^p ≠ x mod f. */
                      /* x^p ≡ x means res = [0, 1, 0] */
