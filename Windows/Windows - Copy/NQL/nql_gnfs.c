...
/** GNFS_POLY_ALPHA(f, bound) — compute α-value of polynomial f.
 *  α(f) ≈ -Σ_{p prime ≤ bound} (log(p) * (number of roots of f mod p)) / (p - 1)
 *  Lower α = better polynomial. Typical bound: 2000. */
 ...
 *  = |Σ_{i=0}^{d} c_i * a^i * (-b)^{d-i}| where f(x) = Σ c_i x^i.
...
static NqlValue nql_fn_gnfs_algebraic_norm(const NqlValue* args, uint32_t ac, void* ctx) {
    ...
    int d = f->degree;
    /* Compute Σ c_i * a^i * (-b)^{d-i} using BigNum */
...
typedef struct {
    int       width, height;
    float*    rat_sieve;       /* heap, width × height */
    float*    alg_sieve;       /* heap, width × height */
    uint64_t  special_q;
    uint64_t  special_r;
...
static NqlValue nql_fn_gnfs_sieve_init_norms(const NqlValue* args, uint32_t ac, void* ctx) {
    ...
    for (int bj = 0; bj < s->height; ++bj) {
        for (int ai = 0; ai < s->width; ++ai) {
            ...
            /* Algebraic: |(-b)^d * f(a/b)| = |Σ c_i * a^i * (-b)^(d-i)| */
            {
                double alg_est = 0.0;
                double a_pow = 1.0;
...
static NqlValue nql_fn_gnfs_sieve_and_collect(const NqlValue* args, uint32_t ac, void* ctx) {
    ...
    for (int bj = 0; bj < s->height; ++bj) {
        for (int ai = 0; ai < s->width; ++ai) {
            ...
            /* Algebraic norm |(-b)^d * f(a/b)| = |Σ c_i * a^i * (-b)^(d-i)| exact int64 */
 ...
 *  First checks divisibility via f(a/b) mod p = 0, i.e., Σ c_i * a^i * (-b)^(d-i) ≡ 0 mod p.
...
static NqlValue nql_fn_gnfs_block_lanczos(const NqlValue* args, uint32_t ac, void* ctx) {
    ...
    /* Working vectors — each is n × 64 bits (one uint64 per row, 64 parallel vectors) */
 ...
 *  Computes x = ∏ (a_i - b_i * m) mod N — the raw product, NOT the sqrt. */
 ...
 *  halve (guaranteed even by GF(2) dependency), compute y = ∏ p^(sum/2) mod N.
...
static NqlValue nql_fn_gnfs_alg_sqrt(const NqlValue* args, uint32_t ac, void* ctx) {
    ...
    /* Compute y = ∏ rat_prime[i]^(rat_sum[i]/2) * ∏ alg_prime[i]^(alg_sum[i]/2) mod N */
