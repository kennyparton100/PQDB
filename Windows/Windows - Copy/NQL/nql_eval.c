...
static NqlValue nql_eval_expr(NqlEvalCtx* ctx, const NqlAstNode* node) {
    ...
    case NQL_NODE_IDENT: {
        /* Check variables FIRST — a LET binding always takes priority */
        ...
        /* Fall back to default column name (for SELECT ... FROM TYPE queries) */
    ...
    case NQL_NODE_BINOP: {
        ...
        /* Auto-promote to the widest numeric tier */
        NqlValType tier = nql_wider_type(&left, &right);

        /* BigNum arithmetic */
        if (tier == NQL_VAL_BIGNUM) {
            BigNum l = nql_val_as_bignum(&left);
            BigNum r = nql_val_as_bignum(&right);
            BigNum res;
            switch (node->d.binop.op) {
                case NQL_TOK_PLUS:    bn_copy(&res, &l); bn_add(&res, &r); return nql_val_from_bignum(&res);
                case NQL_TOK_MINUS:   bn_copy(&res, &l); bn_sub(&res, &r); return nql_val_from_bignum(&res);
                case NQL_TOK_STAR:    bn_mul(&res, &l, &r); return nql_val_from_bignum(&res);
                case NQL_TOK_SLASH:   if (bn_is_zero(&r)) return nql_val_null(); bn_divexact(&res, &l, &r); return nql_val_from_bignum(&res);
                case NQL_TOK_PERCENT: { /* res = l - (l/r)*r */
                    BigNum q; bn_divexact(&q, &l, &r);
                    BigNum qr; bn_mul(&qr, &q, &r);
                    bn_copy(&res, &l); bn_sub(&res, &qr);
                    return nql_val_from_bignum(&res); }
                case NQL_TOK_EQ:      return nql_val_bool(bn_cmp(&l, &r) == 0);
                case NQL_TOK_NEQ:     return nql_val_bool(bn_cmp(&l, &r) != 0);
                case NQL_TOK_LT:      return nql_val_bool(bn_cmp(&l, &r) < 0);
                case NQL_TOK_GT:      return nql_val_bool(bn_cmp(&l, &r) > 0);
                case NQL_TOK_LEQ:     return nql_val_bool(bn_cmp(&l, &r) <= 0);
                case NQL_TOK_GEQ:     return nql_val_bool(bn_cmp(&l, &r) >= 0);
                default: break;
            }
            return nql_val_null();
        }
        ...
        if (tier == NQL_VAL_U128) {
            U128 l = nql_val_as_u128(&left);
            U128 r = nql_val_as_u128(&right);
            switch (node->d.binop.op) {
                case NQL_TOK_PLUS:    return nql_val_from_u128(u128_add(l, r));
                case NQL_TOK_MINUS:   return nql_val_from_u128(u128_sub(l, r));
                case NQL_TOK_STAR:    { /* No native u128*u128; promote to BigNum for multiply */
                    BigNum bl, br, bres; bn_from_u128(&bl, l); bn_from_u128(&br, r);
                    bn_mul(&bres, &bl, &br); return nql_val_from_bignum(&bres); }
                case NQL_TOK_SLASH:   return u128_is_zero(r) ? nql_val_null() : nql_val_from_u128(u128_div(l, r));
                case NQL_TOK_PERCENT: return u128_is_zero(r) ? nql_val_null() : nql_val_from_u128(u128_mod(l, r));
                case NQL_TOK_EQ:      return nql_val_bool(u128_eq(l, r));
                case NQL_TOK_NEQ:     return nql_val_bool(!u128_eq(l, r));
                case NQL_TOK_LT:      return nql_val_bool(u128_lt(l, r));
                case NQL_TOK_GT:      return nql_val_bool(u128_gt(l, r));
                case NQL_TOK_LEQ:     return nql_val_bool(u128_le(l, r));
                case NQL_TOK_GEQ:     return nql_val_bool(u128_le(r, l));
                default: break;
            }
            return nql_val_null();
        }
...
static void nql_val_to_str(const NqlValue* v, char* buf, size_t bufsz) {
    switch (v->type) {
        ...
        case NQL_VAL_U128:   u128_to_str(v->v.u128val, buf, bufsz); break;
        case NQL_VAL_BIGNUM: bn_to_str(&v->v.bnval, buf, bufsz); break;
        case NQL_VAL_ARRAY:  nql_array_to_str(v->v.array, buf, bufsz); break;
        case NQL_VAL_POLY:   nql_poly_to_str((NqlPoly*)v->v.qs_ptr, buf, bufsz); break;
        case NQL_VAL_QS_FACTOR_BASE:  snprintf(buf, bufsz, "<FactorBase:%d primes>", v->v.qs_ptr ? ((QsFactorBase*)v->v.qs_ptr)->count : 0); break;
        case NQL_VAL_QS_SIEVE:        snprintf(buf, bufsz, "<SieveBuffer:%d>", v->v.qs_ptr ? ((QsSieveBuffer*)v->v.qs_ptr)->total_size : 0); break;
        case NQL_VAL_QS_TRIAL_RESULT: snprintf(buf, bufsz, "<TrialResult>"); break;
        case NQL_VAL_QS_RELATION_SET: snprintf(buf, bufsz, "<RelationSet:%d rels>", v->v.qs_ptr ? ((QsRelationSet*)v->v.qs_ptr)->count : 0); break;
        case NQL_VAL_QS_GF2_MATRIX:   snprintf(buf, bufsz, "<GF2Matrix:%dx%d>", v->v.qs_ptr ? ((QsGf2Matrix*)v->v.qs_ptr)->rows : 0, v->v.qs_ptr ? ((QsGf2Matrix*)v->v.qs_ptr)->cols : 0); break;
        case NQL_VAL_QS_DEPENDENCY:    snprintf(buf, bufsz, "<Dependency>"); break;
    }
    buf[bufsz - 1u] = '\0';
}
...
/* ══════════════════════════════════════════════════════════════════════
 * IMPERATIVE STATEMENT EXECUTOR
 *
 * Recursively walks AST nodes for PRINT, ASSERT, IF_STMT, FOR,
 * FOR_EACH, WHILE, BREAK, CONTINUE, LET, and bare expressions.
 * ══════════════════════════════════════════════════════════════════════ */

#define NQL_LOOP_MAX_ITERATIONS 1000000u

/** Execution status flags for loop control flow. */
typedef enum {
    NQL_FLOW_NORMAL,
    NQL_FLOW_BREAK,
    NQL_FLOW_CONTINUE
...
static NqlFlowStatus g_nql_flow = NQL_FLOW_NORMAL;

/** Execute a single statement AST node. */
static void nql_exec_stmt(NqlEvalCtx* ctx, const NqlAstNode* node) {
    if (!node || g_nql_flow == NQL_FLOW_BREAK) return;

    switch (node->type) {

    case NQL_NODE_PRINT: {
        for (uint32_t i = 0u; i < node->d.print.expr_count; ++i) {
            NqlValue val = nql_eval_expr(ctx, node->d.print.exprs[i]);
            char buf[512];
    ...
    case NQL_NODE_ASSERT: {
        NqlValue cond = nql_eval_expr(ctx, node->d.assert_stmt.cond);
        if (!nql_val_is_truthy(&cond)) {
            if (node->d.assert_stmt.message) {
                NqlValue msg = nql_eval_expr(ctx, node->d.assert_stmt.message);
                char buf[256];
                nql_val_to_str(&msg, buf, sizeof(buf));
                printf("ASSERT FAILED: %s\n", buf);
            } else {
                printf("ASSERT FAILED at pos %u\n", node->token.pos);
            }
        }
        break;
    }
    ...
    case NQL_NODE_BREAK:
        g_nql_flow = NQL_FLOW_BREAK;
        break;

    case NQL_NODE_CONTINUE:
        g_nql_flow = NQL_FLOW_CONTINUE;
        break;

    case NQL_NODE_LET: {
        NqlValue val = nql_eval_expr(ctx, node->d.let.value);
        nql_vars_set(ctx->vars, node->d.let.name, val);
        break;
    }

    case NQL_NODE_IF_STMT: {
        NqlValue cond = nql_eval_expr(ctx, node->d.if_stmt.cond);
        if (nql_val_is_truthy(&cond)) {
            for (uint32_t i = 0u; i < node->d.if_stmt.then_count; ++i) {
                nql_exec_stmt(ctx, node->d.if_stmt.then_body[i]);
                if (g_nql_flow != NQL_FLOW_NORMAL) break;
            }
        } else {
            for (uint32_t i = 0u; i < node->d.if_stmt.else_count; ++i) {
                nql_exec_stmt(ctx, node->d.if_stmt.else_body[i]);
                if (g_nql_flow != NQL_FLOW_NORMAL) break;
            }
        }
        break;
    ...
    case NQL_NODE_FOR: {
        NqlValue lo_val = nql_eval_expr(ctx, node->d.for_loop.from_expr);
        NqlValue hi_val = nql_eval_expr(ctx, node->d.for_loop.to_expr);
        int64_t lo = nql_val_as_int(&lo_val);
        int64_t hi = nql_val_as_int(&hi_val);
        uint64_t iterations = 0u;
        for (int64_t i = lo; i <= hi && iterations < NQL_LOOP_MAX_ITERATIONS; ++i, ++iterations) {
            nql_vars_set(ctx->vars, node->d.for_loop.var_name, nql_val_int(i));
            for (uint32_t s = 0u; s < node->d.for_loop.body_count; ++s) {
                nql_exec_stmt(ctx, node->d.for_loop.body[s]);
                if (g_nql_flow != NQL_FLOW_NORMAL) break;
            }
            if (g_nql_flow == NQL_FLOW_BREAK) { g_nql_flow = NQL_FLOW_NORMAL; break; }
            if (g_nql_flow == NQL_FLOW_CONTINUE) { g_nql_flow = NQL_FLOW_NORMAL; }
        }
        break;
    }

    case NQL_NODE_FOR_EACH: {
        NqlValue source = nql_eval_expr(ctx, node->d.for_each.source);
        if (source.type == NQL_VAL_ARRAY && source.v.array) {
            NqlArray* arr = source.v.array;
            for (uint32_t i = 0u; i < arr->length && i < NQL_LOOP_MAX_ITERATIONS; ++i) {
                nql_vars_set(ctx->vars, node->d.for_each.var_name, arr->items[i]);
                for (uint32_t s = 0u; s < node->d.for_each.body_count; ++s) {
                    nql_exec_stmt(ctx, node->d.for_each.body[s]);
                    if (g_nql_flow != NQL_FLOW_NORMAL) break;
                }
                if (g_nql_flow == NQL_FLOW_BREAK) { g_nql_flow = NQL_FLOW_NORMAL; break; }
                if (g_nql_flow == NQL_FLOW_CONTINUE) { g_nql_flow = NQL_FLOW_NORMAL; }
    ...
    case NQL_NODE_WHILE: {
        uint64_t iterations = 0u;
        while (iterations < NQL_LOOP_MAX_ITERATIONS) {
            NqlValue cond = nql_eval_expr(ctx, node->d.while_loop.cond);
            if (!nql_val_is_truthy(&cond)) break;
            for (uint32_t s = 0u; s < node->d.while_loop.body_count; ++s) {
                nql_exec_stmt(ctx, node->d.while_loop.body[s]);
                if (g_nql_flow != NQL_FLOW_NORMAL) break;
            }
            if (g_nql_flow == NQL_FLOW_BREAK) { g_nql_flow = NQL_FLOW_NORMAL; break; }
            if (g_nql_flow == NQL_FLOW_CONTINUE) { g_nql_flow = NQL_FLOW_NORMAL; }
    ...
    default: {
        ...
        NqlValue val = nql_eval_expr(ctx, node);
        char buf[512];
        nql_val_to_str(&val, buf, sizeof(buf));
        printf("= %s\n", buf);
        break;
