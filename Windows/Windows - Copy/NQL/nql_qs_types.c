 ...
 *
 * Internal arithmetic uses U128 where overflow is possible (see plan §A).
...
static NqlValue nql_fn_factor_base_build(const NqlValue* args, uint32_t argc, void* ctx) {
     ...
     * Only one root class because x²-N mod 2 has exactly one residue class. */
    fb->primes[0] = 2u;
...
/** DEPENDENCY_EXTRACT(dep, rels, fb, N) — compute x²≡y² and try GCD.
