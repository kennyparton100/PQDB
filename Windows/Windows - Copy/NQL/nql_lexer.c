...
static const NqlKeyword g_nql_keywords[] = {
    ...
    { "EACH",      NQL_TOK_EACH },
    { "PRINT",     NQL_TOK_PRINT },
    { "ASSERT",    NQL_TOK_ASSERT },
    { "BREAK",     NQL_TOK_BREAK },
    { "CONTINUE",  NQL_TOK_CONTINUE },
    { "RETURN",    NQL_TOK_RETURN },
};
...
static bool nql_lex(const char* src, NqlTokenList* tl) {
    ...
    while (*p != '\0') {
        ...
        /* ── Numeric literal (int64 / U128 / BigNum / float) ── */
        if (nql_is_digit(*p)) {
            const char* num_start = p;
            while (nql_is_digit(*p)) { ++p; ++pos; }
            /* Check for float */
            if (*p == '.' && nql_is_digit(p[1])) {
                ...
                /* Parse float from the digit span */
                char fbuf[128];
                uint32_t flen = t->length < 127u ? t->length : 127u;
                memcpy(fbuf, num_start, flen); fbuf[flen] = '\0';
                t->val.fval = strtod(fbuf, NULL);
            } else {
                t->length = (uint32_t)(p - num_start);
                /* Parse integer: try int64 first, promote if needed */
                bool overflow = false;
                int64_t val = 0;
                for (const char* d = num_start; d < p; ++d) {
                    int64_t prev = val;
                    val = val * 10 + (*d - '0');
                    if (val / 10 != prev) { overflow = true; break; }
                }
                if (!overflow) {
                    t->type = NQL_TOK_INT;
                    t->val.ival = val;
                } else {
                    /* Too large for int64 — store as NQL_TOK_INT with the
                     * raw digit span preserved in start/length for the parser
                     * to re-parse into U128 or BigNum at AST construction time. */
                    t->type = NQL_TOK_INT;
                    t->val.ival = 0; /* sentinel: parser checks length > 18 */
                }
            }
            ++tl->count;
            continue;
