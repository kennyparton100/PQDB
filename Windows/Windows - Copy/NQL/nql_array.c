...
/** Deep copy an NqlValue including array contents. */
static NqlValue nql_val_deep_copy(const NqlValue* src) {
    if (src->type == NQL_VAL_ARRAY && src->v.array) {
        return nql_val_array(nql_array_copy(src->v.array));
    }
    return *src; /* scalar types are value-copied */
}
...
/** ARRAY_UNIQUE(a) — remove duplicates (preserves order, O(n²)) */
...
/** ARRAY_SET_INPLACE(a, index, value) — modify array in place for performance */
static NqlValue nql_fn_array_set_inplace(const NqlValue* args, uint32_t argc, void* ctx) {
    (void)ctx; (void)argc;
    if (args[0].type != NQL_VAL_ARRAY || !args[0].v.array) return nql_val_null();
    int64_t idx = nql_val_as_int(&args[1]);
    NqlArray* a = args[0].v.array;
    if (idx < 0 || (uint32_t)idx >= a->length) return args[0];
    a->items[(uint32_t)idx] = args[2];
    return args[0];
}
