/**
 * cpss_database.c - Multi-shard database routing and orchestration.
 *
 * This module manages a collection of CPSS stream files (shards) that logically 
 * represent a single contiguous prime number database. It handles opening a 
 * directory of shard files, sorting them by their numeric range, and routing 
 * cross-shard queries (like is_prime, next_prime, count_range) to the correct 
 * underlying CPSSStream instance using binary search over the shard boundaries.
 * 
 * It also encapsulates the high-level hot/cold performance logic for counting 
 * primes across segment boundaries.
 */
 ...
 * - Hot path: uses the hot segment cache and its pre-computed rank index.
 *   This avoids zlib decompression, presieve generation, and full prefix scanning.
 *   Block-level popcounts accelerate the edge calculations.
 * - Cold fallback: falls back to full decompression, presieve generation, 
 *   and bit-by-bit walking via `process_loaded_segment_range`.
 *
 * This is the key primitive for fast boundary-segment counting.
 * Note: Does NOT count wheel primes - the caller is responsible for those.
 */
 ...
 * 
 * This maps a folder containing `PrimeStateV4_<lo>_<hi>.bin` files into a unified 
 * queryable `CPSSDatabase` structure.
 * 
 * Fast path: if `.cpss-manifest.json` exists and is valid, uses it to
 * skip directory enumeration and immediately trust the file boundaries.
 * Fallback: uses Win32 `FindFirstFile` to scan the directory.
 * 
