 ...
 *
 * This cache is crucial for the "hot" path, preventing repeated zlib inflates
 * and presieve rebuilds when querying many numbers within the same segment.
 * 
...
typedef struct {
    HotSegmentEntry* entries;   /* Dense array of cache entries */
    size_t cap;                 /* Capacity of the entries array */
    size_t count;               /* Number of occupied slots */
    size_t bytes_used;          /* Total dynamic memory used by cached data */
    size_t budget;              /* Maximum allowed bytes_used before LRU eviction triggers */
    uint64_t use_counter;       /* Monotonic clock for LRU tracking */
    size_t* hash_map;           /* open-addressing hash table: slot index or SIZE_MAX */
    size_t hash_cap;            /* hash table capacity (power of 2) */
} HotSegmentCache;
