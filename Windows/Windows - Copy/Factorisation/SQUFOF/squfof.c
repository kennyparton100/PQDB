...
/* Small multipliers to try — improves success rate on hard inputs by
 * nudging k*N toward continued-fraction cycles that hit a square form sooner. */
    ...
    for (size_t mi = 0u; mi < SQUFOF_MULT_COUNT; ++mi) {
        ...
        if (divisor > 1u && divisor < target) {
            ...
            if (g > 1u && g < target) {
                 ...
                 * SQUFOF only builds a uint64 factor list, so recomposition keeps
                 * the result object consistent after the recursive subcalls. */
                uint64_t product = 1u;
